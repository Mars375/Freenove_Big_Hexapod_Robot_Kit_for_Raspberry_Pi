...
from tachikoma.core.exceptions import SensorReadError
...
class SensorController:
    ...
    async def get_imu_data(self) -> dict:
        """Get IMU orientation data (pitch, roll, yaw)"""
        try:
            imu_raw = await self.read_imu()
            
            # Extract orientation from accelerometer data
            accel = imu_raw.get("accelerometer", [0, 0, 0])
            ...
            # Yaw requires magnetometer (not available), use gyro integration or set to 0
            yaw = 0.0
            ...
            return {
                ...
                "roll": round(roll, 2),
                "yaw": round(yaw, 2),
                "timestamp": imu_raw.get("timestamp")
            }
        except Exception as e:
            logger.error("sensor_controller.imu_data_failed", error=str(e))
            return {"pitch": 0.0, "roll": 0.0, "yaw": 0.0, "timestamp": None}
